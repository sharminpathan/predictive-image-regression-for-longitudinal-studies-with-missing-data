...
def encoder(mode): #mode=1 for images, mode=3 for momentums
  #cnn network
	layer0=Input(shape=(64,64,mode))    #for synthetic dataset
  #layer0=Input(shape=(128,128,mode)) #for oasis dataset
  ...
  #lstm network
	img_inp=Input(shape=(16, 16, 128))          #for synthetic dataset
  #img_inp=Input(shape=(32, 32, 128))         #for oasis dataset
  #lstm0 = Input(shape=(None, 128, 128, 3))   #for oasis dataset
  ...
  a=np.zeros((64,64,3)).tolist()  #(128,128,3) for oasis dataset
  out2=[]
	out1=[]
	out=[]
	for i in range(0,4):
