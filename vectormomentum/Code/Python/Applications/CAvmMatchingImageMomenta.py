...
MatchingImageMomentaVMConfigSpec = {
    ...
    Config.Param(default=0.1,
                 required=True,
                 comment="Regularization weight on image match term, lower values assign higher weight to matching image"),
    'sigmaM':
    Config.Param(default=0.1,
                 required=True,
                 comment="Regularization weight on momenta match term, lower values assign higher weight to matching momenta"),
    'diffOpParams':
...
def MatchingImageMomenta(cf):
    ...
    geodesicState = CAvmHGMCommon.HGMResidualState(I0, p0, imGrid, mType, cf.vectormomentum.diffOpParams[0], cf.vectormomentum.diffOpParams[1], cf.vectormomentum.diffOpParams[2], s, cf.optim.NIterForInverse, 1.0, cf.vectormomentum.sigmaM, cf.vectormomentum.sigmaI, cf.optim.stepSize, integMethod = cf.optim.integMethod)
...
def MatchingImageMomentaComputeEnergy(geodesicState, m0, J1, n1):
    ...
    geodesicState.diffOp.applyInverseOperator(geodesicState.p)
    vecEnergy = 0.5*ca.Dot(geodesicState.p0, geodesicState.p)/(float(geodesicState.p0.nVox())*geodesicState.SigmaIntercept*geodesicState.SigmaIntercept) 
